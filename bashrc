...
bash_prompt() {
	...
	local PROMT_USER=$"$TEXT_FORMAT_1 PapaBollie Terminal"
	local PROMT_HOST=$"$TEXT_FORMAT_2 OR ELSE 🔪 "
	local PROMT_PWD=$"$TEXT_FORMAT_3 \${NEW_PWD} "
	local PROMT_INPUT=$"$PROMT_FORMAT 🏍️  "
	...
	*)
	...
	############################################################################
	PS1="$TITLEBAR\n${PROMT_USER}${SEPARATOR_1}${SEPARATOR_2}${PROMT_PWD}${SEPARATOR_3}${PROMT_INPUT}"
...
__conda_setup="$('/home/jensen/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/jensen/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/jensen/anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/jensen/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<

export PATH="$PATH:/home/jensen/flutter/bin"
